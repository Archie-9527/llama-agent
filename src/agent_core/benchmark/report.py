...
def write_report(
    ...
    lines = [
        ...
        f"- 测量样本持久化总量：{_mib(summary.get('total_persistent_storage_bytes'))}",
        ...
        f"- R2 因收益不足跳过的记录数：{summary.get('context_compaction_skipped_count', 0)}",
        f"- R2 被压缩原文总量：{summary.get('context_compaction_source_bytes', 0)} 字节",
        f"- R2 压缩表示总量：{summary.get('context_compacted_bytes', 0)} 字节",
