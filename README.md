...
系统支持单任务执行、持久化多轮会话、本地工具调用、全屏交互式 CLI、运行遥测和可重复 Benchmark。在此基础上，项目实现了两项面向长生命周期Agent 的上下文优化，当前优化属于Agent应用层上下文优化，能减少输入Token、Prompt预填充开销和逻辑KV占用。

## 目录

- [1. 项目说明](#1-项目说明)
- [2. R0、R1、R2 优化设计](#2-r0-r1-r2-优化设计)
- [3. Benchmark 设计与测试结果](#3-benchmark-设计与测试结果)
- [4. 安装、配置与使用](#4-安装-配置与使用)
...
项目面向长周期 Agent 持续增长的上下文实现了两项优化：

- **R1：工具输出虚拟化**。针对日志、文件和数据库查询等大型工具结果，将原文外置到 ArtifactStore，模型只接收摘要、首尾预览和检索句柄；需要更多内容时，再通过 Artifact 工具搜索或分段读取原文。该机制避免一次大型工具输出占满Prompt。
- **R2：生命周期上下文管理**。针对长任务和多轮对话不断累积的历史，根据内容的重要性、时效性和访问阶段，进行选择、压缩或归档历史执行记录与会话轮次。

详细设计参见[第 2 章：R0、R1、R2 优化设计](#2-r0-r1-r2-优化设计)。

项目同时实现了 **Benchmark 自动化框架**：使用本地确定性 Fixture，在独立 Python子进程中运行每个样本，隔离模型、KV 状态、Checkpoint 和各类存储；R0、R1、R2使用相同模型、任务和评价规则，并自动生成原始数据、失败记录、报告和图表。

最终测试结果表明：

- **R1 适合大型工具输出场景**。在包含大型工具调用测试中任务成功率保持 97.78%，相对 R0（初始版本） 输入 Token 减少 27.59%、峰值逻辑 KV Token 减少 90.35%、Checkpoint 总量减少22.42%、平均端到端耗时减少 4.51%。其中 64 KiB 大文件用例的单样本输入 Token 从 21,572 降至 3,960，平均耗时从 30.93 秒降至 15.74 秒。
- **R2 适合累计执行记录和超长会话场景**。在累计上下文压力测试中，相对 R1 输入 Token 减少 7.70%、峰值逻辑 KV Token 减少 17.42%、Checkpoint 总量减少35.06%、平均耗时减少 7.43%，成功率保持 100%；在 16/32 轮长会话测试中，成功率由 R1 的 50% 提升至 100%，输入 Token 减少 10.24%，平均耗时减少18.21%。

详细测试方法、用例和图表参见[第 3 章：Benchmark 设计与测试结果](#3-benchmark-设计与测试结果)。
