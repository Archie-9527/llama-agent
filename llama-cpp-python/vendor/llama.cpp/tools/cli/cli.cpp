...
struct cli_context {
    // 服务器上下文，核心对象，负责管理模型加载、推理请求处理等功能
    server_context ctx_server;
    // 对话历史，存储多轮对话历史，每条消息包含角色（用户或助手）和内容，用于构建提示词和生成响应，使用Chat API标准格式
    json messages = json::array();
    // 输入的媒体文件(图片或音频等)
    std::vector<raw_buffer> input_files;
    // 默认推理参数 (可以被每次生成时的特定参数覆盖)
    task_params defaults;
    // 是否打印完整的提示词（包括系统提示词等）到控制台，默认为 false 以避免泄露敏感信息
    bool verbose_prompt;

    // 控制台显示当前正在加载模型的动画，直到模型加载完成
    // thread for showing "loading" animation
    std::atomic<bool> loading_show;

    cli_context(const common_params & params) {
        defaults.sampling    = params.sampling;     // 采样参数 
        defaults.speculative = params.speculative;  // 投机采样配置
        defaults.n_keep      = params.n_keep;       // KV Cache保留的Token数量
        defaults.n_predict   = params.n_predict;    // 预测的Token数量上限
        defaults.antiprompt  = params.antiprompt;   // 反提示词列表

        defaults.stream = true; // make sure we always use streaming mode
        defaults.timings_per_token = true; // in order to get timings even when we cancel mid-way
        // defaults.return_progress = true; // TODO: show progress

        // 调试开关，是否在控制台打印完整的提示词（包括系统提示词等），默认为 false 以避免泄露敏感信息
        verbose_prompt = params.verbose_prompt;
    }

    // 生成文本的主要函数，负责与服务器交互，发送生成请求，并处理生成结果
    std::string generate_completion(result_timings & out_timings) {
        ...
        auto chat_params = format_chat();
        {
            ...
            task.index      = 0;
            task.params     = defaults;           // copy，每次生成都从默认参数开始，可以被 chat_params 中的特定参数覆盖
            ...
            // 把推理任务发布到服务器的任务队列中，等待服务器处理并生成响应
        ...
        while (result) {
            ...
            if (res_partial) {
                ...
                for (const auto & diff : res_partial->oaicompat_msg_diffs) {
                    // 收到正式的增量内容时，如果之前处于思考状态，则先结束思考状态的显示，再显示新的内容增量
                    ...
                    // 收到思考内容增量时，切换到思考状态的显示，如果之前不是思考状态，则先显示思考开始的提示
            ...
            // 生成完毕的结果可能包含一些最终的增量内容，或者在某些情况下没有增量内容（例如生成被截断了），因此我们也需要处理 final 结果中的增量内容
            if (res_final) {
    ...
    // 格式话对话历史和系统提示词等信息，生成最终用于推理的提示词字符串
...
// 文件路径自动补全的回调函数，支持以 "/" 开头的命令和文件路径补全
...
int llama_cli(int argc, char ** argv) {
    ...
    // 后台推理线程，负责与服务器交互，发送生成请求，并处理生成结果
